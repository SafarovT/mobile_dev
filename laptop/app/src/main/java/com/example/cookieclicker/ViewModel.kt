    ...
    private fun onChange() {
        viewModelScope.launch {
            state.update {
                ...
                var newNextCookiesMessage = it.nextCookiesMessage
                if (newCookies == newNextCookiesMessage) {
                    newNextCookiesMessage += INCOME_MESSAGE_NUMBER
                    event.emit(CookieEvent.CookiesIncomeNotification)
                }
                it.copy(
                    cookies = newCookies,
                    nextCookiesMessage = newNextCookiesMessage,
                    time = it.time + 1,
